...
class BoardController: NSObject,
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
    // ...
      if let numTimesGuessed = settings[kNumGuessesKey] as? Int {
          self.numRows = numTimesGuessed
      } else {
          numRows = 6
      }
    // END YOUR CODE HERE
  ...
  private func resetBoardWithCurrentSettings(with settings: [String: Any]) {
        
      if let isResetCurrentBoardSetting = settings[kIsAlienWordleKey] as? Bool {
          isResetCurrentBoard = isResetCurrentBoardSetting
      }
  }
